...
from runtime_paths import DATA_DIR, RESOURCE_BASE_DIR, STATIC_DIR, get_runtime_data_path
...
LOCAL_BUILDING_SNAPSHOT_PATH = get_runtime_data_path("douarnenez_buildings_geopf_snapshot.geojson", copy_if_missing=True)
...
ORTHO_TILE_SOURCE_FALLBACK_SIGNATURE = "unknown-source"
ORTHO_TILE_GENERATION_MANIFEST_NAME = "_generation.json"
...
ORTHO_TILE_VERSION_SAMPLE_COUNT = 8
...
def normalize_ortho_source_signature(signature: str | None) -> str:
    raw_signature = re.sub(r"[^a-zA-Z0-9._-]+", "-", str(signature or "").strip()).strip("-")
    return raw_signature or ORTHO_TILE_SOURCE_FALLBACK_SIGNATURE


def get_current_ortho_source_signature() -> str:
    return normalize_ortho_source_signature(ortho_source_signature(load_ortho_source_state()))


def get_ortho_tile_cache_generation_dir(source_signature: str | None) -> Path:
    return ORTHO_TILE_CACHE_DIR / normalize_ortho_source_signature(source_signature)


def get_ortho_generation_manifest_path(source_signature: str | None) -> Path:
    return get_ortho_tile_cache_generation_dir(source_signature) / ORTHO_TILE_GENERATION_MANIFEST_NAME


def load_ortho_generation_manifest(source_signature: str | None) -> dict[str, object] | None:
    manifest_path = get_ortho_generation_manifest_path(source_signature)
...
def save_ortho_generation_manifest(source_signature: str | None, payload: dict[str, object]) -> Path:
    generation_dir = get_ortho_tile_cache_generation_dir(source_signature)
    generation_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = generation_dir / ORTHO_TILE_GENERATION_MANIFEST_NAME
    temp_path = manifest_path.with_name(f"{manifest_path.stem}.{time.time_ns()}.tmp")
    temp_path.write_text(json.dumps(payload, ensure_ascii=True, separators=(",", ":")), encoding="utf-8")
    temp_path.replace(manifest_path)
    return manifest_path


def get_flat_ortho_tile_cache_path(z: int, x: int, y: int) -> Path:
    return ORTHO_TILE_CACHE_DIR / f"zoom_{z}" / str(x) / f"{y}.jpg"


def get_ortho_tile_cache_path(z: int, x: int, y: int, source_signature: str | None = None) -> Path:
    cache_generation_dir = get_ortho_tile_cache_generation_dir(source_signature or get_current_ortho_source_signature())
    return cache_generation_dir / f"zoom_{z}" / str(x) / f"{y}.jpg"
...
def iter_legacy_ortho_tile_paths(z: int, x: int, y: int) -> list[Path]:
    return [
        get_flat_ortho_tile_cache_path(z, x, y),
        get_legacy_ortho_tile_cache_path(z, x, y),
        get_older_ortho_tile_cache_path(z, x, y),
    ]


def resolve_legacy_ortho_tile_path(z: int, x: int, y: int) -> Path | None:
    for legacy_tile_path in iter_legacy_ortho_tile_paths(z, x, y):
        if legacy_tile_path.exists():
            return legacy_tile_path
    return None
...
def build_ortho_generation_manifest(
    ...
    *,
    ...
    if previous_signature:
        manifest["previous_signature"] = previous_signature
    if verification:
        manifest["verification"] = verification
    return manifest


def resolve_cached_ortho_tile_path(
    z: int,
    x: int,
    y: int,
    *,
    ...
    tile_path = get_ortho_tile_cache_path(z, x, y, source_signature)
    if tile_path.exists():
        return tile_path
...
def attach_legacy_ortho_tile_to_signature(
    z: int,
    x: int,
    y: int,
    ...
    legacy_tile_path = resolve_legacy_ortho_tile_path(z, x, y)
    if legacy_tile_path is not None:
...
def migrate_legacy_ortho_tiles(
    tiles: list[tuple[int, int, int]],
    source_signature: str | None,
) -> int:
    moved_tiles = 0
    for zoom, x_tile, y_tile in tiles:
        if resolve_cached_ortho_tile_path(zoom, x_tile, y_tile, source_signature=source_signature) is not None:
            continue
        if attach_legacy_ortho_tile_to_signature(zoom, x_tile, y_tile, source_signature) is not None:
            moved_tiles += 1
    return moved_tiles


def pick_spaced_items(items: list, count: int) -> list:
    if not items:
        return []
    if count <= 0:
        return []
    ...
    for item_index in range(count):
        index = round(item_index * stride)
        if index in used_indexes:
            continue
        used_indexes.add(index)
        spaced_items.append(items[index])
    return spaced_items


def select_spaced_tile_samples(
    tiles: list[tuple[int, int, int]],
    *,
    sample_count: int,
    ...
    tiles_by_zoom: dict[int, list[tuple[int, int, int]]] = {}
    for tile in tiles:
        tiles_by_zoom.setdefault(tile[0], []).append(tile)

    ordered_zooms = sorted(tiles_by_zoom)
    if len(ordered_zooms) >= sample_count:
        sampled_zooms = pick_spaced_items(ordered_zooms, sample_count)
        return [tiles_by_zoom[zoom][len(tiles_by_zoom[zoom]) // 2] for zoom in sampled_zooms]

    allocations = {zoom: 1 for zoom in ordered_zooms}
    remaining_samples = sample_count - len(ordered_zooms)
    zooms_by_size = sorted(ordered_zooms, key=lambda zoom: (-len(tiles_by_zoom[zoom]), zoom))

    while remaining_samples > 0:
        allocated_this_round = False
        for zoom in zooms_by_size:
            if allocations[zoom] >= len(tiles_by_zoom[zoom]):
                continue
            allocations[zoom] += 1
            remaining_samples -= 1
    ...
    for zoom in ordered_zooms:
        ...
        quota = allocations[zoom]
        if quota >= len(zoom_tiles):
            samples.extend(zoom_tiles)
            continue
        if quota == 1:
            samples.append(zoom_tiles[len(zoom_tiles) // 2])
            continue

        samples.extend(pick_spaced_items(zoom_tiles, quota))

    return samples[:sample_count]


def build_ortho_tile_payload_fingerprint(tile_bytes: bytes) -> dict[str, object]:
    return {
        "byte_length": len(tile_bytes),
        "sha256": hashlib.sha256(tile_bytes).hexdigest(),
    }


def compare_ortho_tile_payloads(local_bytes: bytes, remote_bytes: bytes) -> dict[str, object]:
    local_fingerprint = build_ortho_tile_payload_fingerprint(local_bytes)
    remote_fingerprint = build_ortho_tile_payload_fingerprint(remote_bytes)
    return {
        "matches": (
            local_fingerprint["byte_length"] == remote_fingerprint["byte_length"]
            and local_fingerprint["sha256"] == remote_fingerprint["sha256"]
        ),
        "local_byte_length": local_fingerprint["byte_length"],
        "remote_byte_length": remote_fingerprint["byte_length"],
        "local_sha256": local_fingerprint["sha256"],
        "remote_sha256": remote_fingerprint["sha256"],
    }


def compare_remote_tiles_against_local_cache(
    tiles: list[tuple[int, int, int]],
    *,
    tile_path_resolver: Callable[[int, int, int], Path | None],
    comparison_label: str,
    sample_count: int = ORTHO_TILE_VERSION_SAMPLE_COUNT,
) -> dict[str, object]:
    candidate_tiles = [tile for tile in tiles if tile_path_resolver(*tile) is not None]
    sampled_tiles = select_spaced_tile_samples(candidate_tiles, sample_count=min(sample_count, len(candidate_tiles)))
    comparisons: list[dict[str, object]] = []

    for zoom, x_tile, y_tile in sampled_tiles:
        local_tile_path = tile_path_resolver(zoom, x_tile, y_tile)
        if local_tile_path is None:
            continue

        local_bytes = local_tile_path.read_bytes()
        remote_bytes = fetch_remote_ortho_tile(zoom, x_tile, y_tile)
        comparison = compare_ortho_tile_payloads(local_bytes, remote_bytes)
        comparisons.append(
            {
                "tile": [zoom, x_tile, y_tile],
                **comparison,
            }
        )
        if not comparison["matches"]:
            return {
                ...
                "cached_candidates": len(candidate_tiles),
                "comparison_label": comparison_label,
                "comparison_mode": "tile-bytes-sha256",
                "comparisons": comparisons,
            }

    return {
        "equivalent": bool(comparisons),
        "sampled": len(comparisons),
        "cached_candidates": len(candidate_tiles),
        "comparison_label": comparison_label,
        "comparison_mode": "tile-bytes-sha256",
...
def compare_remote_tiles_against_cached_generation(
    tiles: list[tuple[int, int, int]],
    *,
    previous_signature: str,
    sample_count: int = ORTHO_TILE_VERSION_SAMPLE_COUNT,
) -> dict[str, object]:
    return compare_remote_tiles_against_local_cache(
        tiles,
        tile_path_resolver=lambda z, x, y: resolve_cached_ortho_tile_path(
            z,
            x,
            y,
            source_signature=previous_signature,
        ),
        comparison_label=f"generation:{previous_signature}",
        sample_count=sample_count,
    )


def compare_remote_tiles_against_legacy_cache(
    tiles: list[tuple[int, int, int]],
    *,
    sample_count: int = ORTHO_TILE_VERSION_SAMPLE_COUNT,
) -> dict[str, object]:
    return compare_remote_tiles_against_local_cache(
        tiles,
        tile_path_resolver=resolve_legacy_ortho_tile_path,
        comparison_label="legacy-unversioned",
        sample_count=sample_count,
    )


def copy_or_link_tile(source_path: Path, target_path: Path) -> None:
...
def clone_cached_ortho_generation(
    tiles: list[tuple[int, int, int]],
    *,
    previous_signature: str,
    ...
    cloned_tiles = 0
    for zoom, x_tile, y_tile in tiles:
        source_path = resolve_cached_ortho_tile_path(zoom, x_tile, y_tile, source_signature=previous_signature)
        if source_path is None:
            continue
        target_path = get_ortho_tile_cache_path(zoom, x_tile, y_tile, current_signature)
        if target_path.exists():
            continue
        copy_or_link_tile(source_path, target_path)
        cloned_tiles += 1
    return cloned_tiles
...
def ensure_cached_ortho_tile(
    ...
    *,
    ...
    normalized_signature = normalize_ortho_source_signature(source_signature or get_current_ortho_source_signature())
    tile_path = resolve_cached_ortho_tile_path(z, x, y, source_signature=normalized_signature) or get_ortho_tile_cache_path(
        z,
        x,
        y,
...
def collect_missing_ortho_tiles(
    ...
    *,
    ...
    for tile in iter_ortho_tiles(bounds, min_zoom, max_zoom):
        total_tiles += 1
        if resolve_cached_ortho_tile_path(*tile, source_signature=source_signature) is not None:
            cached_tiles += 1
        else:
...
def ensure_frontend_ortho_tile_cache(bounds: dict[str, float]) -> None:
    ...
    local_signature = normalize_ortho_source_signature(ortho_source_signature(local_source_state))
    all_tiles = list(iter_ortho_tiles(bounds, ORTHO_TILE_FRONT_MIN_ZOOM, ORTHO_TILE_FRONT_MAX_ZOOM))
    source_signature_changed = False
    current_signature = local_signature
    force_refresh_generation = False
    clone_verification: dict[str, object] | None = None
    legacy_verification: dict[str, object] | None = None
    cloned_tiles = 0

    if local_signature != ORTHO_TILE_SOURCE_FALLBACK_SIGNATURE:
        migrated_tiles = migrate_legacy_ortho_tiles(all_tiles, local_signature)
        if migrated_tiles:
            print(
                f"Migration du cache ortho historique vers la generation {local_signature[:12]}: "
                f"{migrated_tiles} tuiles rattachees."
            )
            if load_ortho_generation_manifest(local_signature) is None:
                save_ortho_generation_manifest(
                    local_signature,
                    build_ortho_generation_manifest(
                        local_source_state,
    ...
    try:
        ...
        remote_signature = normalize_ortho_source_signature(ortho_source_signature(remote_source_state))
        save_ortho_source_state(remote_source_state)
        current_signature = remote_signature

        if remote_signature != ORTHO_TILE_SOURCE_FALLBACK_SIGNATURE:
            if local_signature == ORTHO_TILE_SOURCE_FALLBACK_SIGNATURE:
                try:
                    legacy_verification = compare_remote_tiles_against_legacy_cache(all_tiles)
                except Exception as error:
                    legacy_verification = {
                        "equivalent": False,
                        "sampled": 0,
                ...
                if legacy_verification.get("equivalent"):
                    migrated_tiles = migrate_legacy_ortho_tiles(all_tiles, remote_signature)
                    if migrated_tiles:
                        print(
                            "Le cache ortho historique non versionne correspond a la signature distante actuelle. "
                            f"{migrated_tiles} tuiles ont ete rattachees a la generation {remote_signature[:12]}."
                        )
                    if load_ortho_generation_manifest(remote_signature) is None:
                        save_ortho_generation_manifest(
                            remote_signature,
                            build_ortho_generation_manifest(
                                remote_source_state,
                                source_signature=remote_signature,
                                strategy="legacy-validated",
                                verification=legacy_verification,
                            ),
                        )
                elif int(legacy_verification.get("cached_candidates", 0)) > 0:
                    sampled = int(legacy_verification.get("sampled", 0))
                    if sampled > 0:
                        print(
                            "Le cache ortho historique non versionne ne correspond pas a la signature distante actuelle. "
                            "La zone geree sera telechargee dans une nouvelle generation versionnee."
                        )
                    else:
                        print(
                            "Le cache ortho historique existe mais n'a pas pu etre verifie contre la signature distante. "
                            "La zone geree sera telechargee dans une nouvelle generation versionnee."
                        )
                ...
                print(
                    ...
                    f"Generation locale precedente: {local_signature[:12]}. "
                    f"Nouvelle generation distante: {remote_signature[:12]}."
                )

                try:
                    clone_verification = compare_remote_tiles_against_cached_generation(
                        all_tiles,
                        previous_signature=local_signature,
                    )
                except Exception as error:
                    clone_verification = {
                        "equivalent": False,
                ...
                if clone_verification.get("equivalent"):
                    ...
                    save_ortho_generation_manifest(
                        ...
                        build_ortho_generation_manifest(
                            remote_source_state,
                            source_signature=remote_signature,
                            strategy="validated-clone",
                            previous_signature=local_signature,
                            verification=clone_verification,
                        ),
                    )
                    print(
                        "Les tuiles echantillonnees sont identiques entre les deux generations "
                        f"({clone_verification.get('sampled', 0)} echantillons verifies). "
                        f"Reutilisation locale de {cloned_tiles} tuiles sans retelechargement complet."
                    )
                else:
                    force_refresh_generation = True
                    sampled = int(clone_verification.get("sampled", 0)) if clone_verification else 0
                    if sampled > 0:
                        print(
                            "Au moins une tuile echantillonnee differe entre les deux generations. "
                            "Le cache local doit etre rafraichi pour la zone geree."
                        )
                    else:
                        print(
                            "Aucune comparaison fiable n'a pu confirmer l'equivalence locale "
                            "entre les deux generations. Un rafraichissement complet est lance."
                        )
            elif local_signature == remote_signature:
    ...
    for tile in tiles_to_process[:probe_count]:
        ...
        try:
            _, status = ensure_cached_ortho_tile(
                *tile,
                force=force_refresh_generation,
                source_signature=current_signature,
            )
    ...
    _, available_tiles, final_missing_tiles = collect_missing_ortho_tiles(
        bounds,
        ORTHO_TILE_FRONT_MIN_ZOOM,
        ORTHO_TILE_FRONT_MAX_ZOOM,
        source_signature=current_signature,
    ...
    if (
        ...
        and not force_refresh_generation
        and load_ortho_generation_manifest(current_signature) is None
    ):
        save_ortho_generation_manifest(
            current_signature,
            build_ortho_generation_manifest(
                load_ortho_source_state(),
                source_signature=current_signature,
                strategy="validated-clone",
                previous_signature=local_signature if local_signature != current_signature else None,
                verification=clone_verification,
            ),
        )
    ...
    print(
        ...
        f"dans la generation {current_signature[:12]}. Statuts: {status_counts}. "
        f"Tuiles encore manquantes: {len(final_missing_tiles)}. Echecs: {failures}."
    )


def send_ortho_tile_response(handler: SimpleHTTPRequestHandler, z: int, x: int, y: int) -> None:
    try:
        tile_path, cache_status = ensure_cached_ortho_tile(z, x, y)
    except Exception as error:
...
def open_browser_when_ready(*, timeout_seconds: int = 180) -> threading.Thread:
    application_url = f"http://{HOST}:{PORT}"
    overview_url = f"{application_url}/api/overview"
    ...
    def _open_browser() -> None:
        ...
        print("Le navigateur n'a pas pu etre ouvert automatiquement dans le delai imparti.")
